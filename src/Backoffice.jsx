...
function Truck() {
  ...
  return _jsxs('section', { children: [_jsx('h2', { children: 'TRUCKSTATUS' }), _jsxs('div', { className: 'truck-panel', children: [
    ...
    _jsx('button', { className: 'bo-primary checkin', onClick: () => setChecked(true), children: checked ? '✓ PLATS DEMO-PUBLICERAD' : '⌖ CHECKA IN TRUCKEN HÄR' }),
...
function Live() {
  ...
  return _jsxs('section', { children: [_jsx('h2', { children: `SÄNDER ${language.live.toUpperCase()}` }), _jsxs('div', { className: 'live-grid', children: [_jsxs('div', { className: 'live-controls', children: [_jsx('div', { className: `live-preview ${live ? 'on' : ''}`, children: live ? `● ${language.live.toUpperCase()} · FÖRHANDSVISNING` : 'KAMERA AV' }), _jsxs('label', { children: ['TITEL', _jsx('input', { value: title, onChange: e => setTitle(e.target.value) })] }), _jsxs('div', { className: 'device-row', children: [_jsx('button', { children: '📷 KAMERA' }), _jsx('button', { children: '🎙 MIKROFON' })] }), _jsx('button', { className: `bo-primary ${live ? 'stop' : ''}`, onClick: () => setLive(!live), children: live ? `STOPPA ${language.live.toUpperCase()}` : `STARTA ${language.live.toUpperCase()}` })] }), _jsxs('div', { className: 'public-live', children: [_jsxs('b', { children: [live ? `● SÄNDER ${language.live.toUpperCase()}` : '○ INTE LIVE', ' · ', live ? '23 TITTARE' : '0 TITTARE'] }), _jsx('h3', { children: title }), _jsx('div', { className: 'chat', children: chatMessages.map((x, i) => _jsx('p', { children: x }, i)) }), _jsxs('form', { onSubmit: e => { e.preventDefault(); if (message) { setChat(previous => [...(Array.isArray(previous) ? previous : []), `du: ${message}`]); setMessage(''); } }, children: [_jsx('input', { value: message, onChange: e => setMessage(e.target.value), placeholder: 'Skriv i chatten' }), _jsx('button', { children: 'SKICKA' })] })] })] }), _jsx('p', { className: 'hint', children: 'DEMO: Video och chatt kopplas senare till en etablerad streamingplattform.' })] });
}
